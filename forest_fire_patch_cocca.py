...
class FireAgent(mesa.Agent):
    ...
    def __init__(self, model, pos, fire_type="fire"):
        ...
        self.color_intensity = 1.0  # 1.0 is bright red, lower values darken
...
class ForestFireModel(mesa.Model):
    def __init__(self, width, height, density, seed=None):
        ...
        self.burned_trees = 0
        self.patches = {}  # Dictionary to track patch state: "green", "burned", etc.
    ...
    def get_patch_grid(self):
        ...
        for (x, y), state in self.patches.items():
            if state == "green":
                ...
                grid[x, y] = 2  # Burned trees
        return grid.T  # Transpose for correct orientation in imshow
...
if __name__ == "__main__":
    ...
    custom_cmap = ListedColormap(["white", "green", "red"])  # 0: empty, 1: green, 2: red
