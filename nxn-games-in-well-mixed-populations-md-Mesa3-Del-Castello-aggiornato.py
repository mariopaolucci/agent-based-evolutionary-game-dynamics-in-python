...
class Player(Agent):
    ...
    def imitative_linear_attraction_rule(self):
        obs = random.choice(self.model.agents)
...
class GameModel(Model):
    ...
    def payoff_for_strategy(self, strategy):
        if self.payoff_to_use == "play-with-one-rd-agent":
            others = [a for a in self.agents if a is not self]
...
# Usiamo batch_run per i plot
# ─── plot_abm ───
def plot_abm(populations, model_kwargs, runs_per_N=1):
    fig, axes = plt.subplots(1, len(populations), figsize=(12, 4))
    fig.subplots_adjust(top=0.88, bottom=0.15, hspace=0.3, left=0.08, right=0.98, wspace=0.25)

    for ax, N in zip(axes, populations):
        n0 = int(0.7 * N)
        params = {
            **model_kwargs,
            'n_of_players_for_each_strategy': [n0, N - n0]
        }
        # batch_run si aspetta ogni parametro come lista
        ...
        # 3) Calcola la media ABM su Step
        df_grouped = df.groupby('Step')['x_agent'].mean()
        t = df_grouped.index
        xs_abm = df_grouped.values

        # 4) Calcola la curva MD analitica
        x0 = df_grouped.iloc[0]
        temp_model = GameModel([n0, N - n0], **model_kwargs)
        rule = model_kwargs.get('decision_rule', DECISION_RULE)
        pm   = model_kwargs.get('payoff_to_use', PAYOFF_TO_USE)
        md_name = f"{rule.replace('-', '_')}_{pm.replace('-', '_')}_md"
        md_func = getattr(temp_model, md_name, temp_model.replicator_dynamics_md)
        xs_md, _ = simulate_md(md_func, x0,
                               model_kwargs['prob_revision'],
                               model_kwargs['noise'],
                               STEPS)
    ...
    # Legenda comune
    handles = [
        plt.Line2D([], [], color='tab:green', alpha=0.6, lw=8),
    ...
    fig.suptitle('Simulazioni del gioco di coordinazione 1-2',
                  fontsize=12)
...
def plot_md(decision_rules, payoff_modes, model_kwargs, runs_per_setting=1):
    """
    ...
    fig.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.5, 0.96),
               ncol=3, frameon=False, fontsize=9)
    fig.suptitle("Strategy Distribution & Mean Dynamics", fontsize=13)
    fig.tight_layout(rect=[0, 0, 1, 0.94]) 
    return fig

# ─── plot per le due regole selezionate ───
def plot_selected_md(model_kwargs, runs_per_setting=1):
    """
    Plot combinato: curva MD analitica (linea tratteggiata) e ABM
    per le sole regole imitative_linear_attraction e imitative_linear_dissatisfaction,
    e per entrambe le modalità di payoff.
    """
    decision_rules = [
        "imitative-linear-attraction",
        "imitative-linear-dissatisfaction"
    ]
    payoff_modes = [
        "play-with-one-rd-agent",
        "use-strategy-expected-payoff"
    ]
    ...
    fig.subplots_adjust(top=0.88, hspace=0.3, wspace=0.2)                         
    for i, rule in enumerate(decision_rules):
        pretty_rule = rule.replace("imitative-", "").replace("-", " ").title()
        for j, pm in enumerate(payoff_modes):
            # predispone modello per la MD analitica
            temp_model = GameModel([700,300],
                                   **{**model_kwargs, 
                                      'decision_rule': rule, 
                                      'payoff_to_use': pm})
            md_func = getattr(
                ...
                temp_model.replicator_dynamics_md
            )
            # calcola MD analitica
            xs_md, _ = simulate_md(md_func, 300/1000,
                                   model_kwargs['prob_revision'],
                                   model_kwargs['noise'], STEPS)
            # esegue batch ABM
            params = {
                **model_kwargs,
                'decision_rule': rule,
            ...
            ax.fill_between(t, xs_abm, 1, color="tab:orange", alpha=0.6)
            ax.plot(range(len(xs_md)), xs_md, "--k", linewidth=1)
            pretty_pm = pm.replace("play-with-one-rd-agent", "play-with-one-rd-agent")\
                          .replace("use-strategy-expected-payoff", "use-strat.-exp.-payoff")
            ax.set_title(f"{pretty_rule}\n{pretty_pm}",
                         fontsize=7, pad=4)
    ...
    fig.legend(handles, labels, loc='upper center',
               ...
               ncol=3, frameon=False, fontsize=9)
    fig.suptitle("Selected Rules: Strategy Distribution & Mean Dynamics", 
                  fontsize=12)
    fig.tight_layout(rect=[0, 0, 1, 0.92])              
    return fig
...
if __name__ == "__main__":
    ...
    plt.show()
    # secondo grafico completo
    fig2 = plot_md(DECISION_RULES, PAYOFF_MODES, model_kwargs)
    plt.show()
    # terzo grafico: solo le due regole selezionate
    fig3 = plot_selected_md(model_kwargs)
    plt.show()
